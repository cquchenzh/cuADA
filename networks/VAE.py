...
from .reciprocal_norm import RN2d
...
class VAE(nn.Module):
    def __init__(self, KERNEL=3,PADDING=1,in_channels = 1,num_classes = 4):
        ...
        self.conv_seq1 = nn.Sequential( nn.Conv2d(in_channels,64,kernel_size=KERNEL,padding=PADDING),
        ...
        self.down4fc2 = nn.Sequential(nn.Conv2d(256, 256, kernel_size=KERNEL, padding=PADDING),
                                      nn.InstanceNorm2d(256),
                                      nn.Tanh())
        self.segdown4_seq = nn.Sequential(nn.Conv2d(256, num_classes, kernel_size=KERNEL, padding=PADDING),)
        ...
        self.down2fc2 = nn.Sequential(nn.Conv2d(128, 128, kernel_size=KERNEL, padding=PADDING),
                                      nn.InstanceNorm2d(128),
                                      nn.Tanh())
        self.segdown2_seq = nn.Sequential(nn.Conv2d(128, num_classes, kernel_size=KERNEL, padding=PADDING),)
        ...
        self.deconv_seq5 = nn.Sequential(nn.Conv2d(64, 64, kernel_size=KERNEL, padding=PADDING),
                                       nn.InstanceNorm2d(64),
                                       nn.ReLU(inplace=True),
                                       nn.Conv2d(64, num_classes, kernel_size=KERNEL, padding=PADDING))
        self.soft = nn.Softmax2d()

        self.upsample2 = nn.Upsample(scale_factor=2,mode='bilinear')
        self.upsample4 = nn.Upsample(scale_factor=4,mode='bilinear')
        self.segfusion = nn.Sequential(nn.Conv2d(num_classes*3, num_classes*3, kernel_size=KERNEL, padding=PADDING),
                                       nn.InstanceNorm2d(num_classes*3),
                                       nn.ReLU(inplace=True),
                                       nn.Conv2d(num_classes * 3, num_classes, kernel_size=KERNEL, padding=PADDING),)
...
class InfoNet(nn.Module):
    def __init__(self, KERNEL=3, PADDING=1):
        ...
        self.info_seq=nn.Sequential(nn.Linear(1024*16*16,256),
                                    nn.ReLU(inplace=True),
                                    nn.Linear(256,6))
    def forward(self, z):
...
class VAEDecode(nn.Module):
    def __init__(self, KERNEL=3, PADDING=1, in_channels=1,num_classes = 4):
        ...
        self.decoderB=nn.Sequential(
            nn.Conv2d(64+num_classes, 128, kernel_size=KERNEL, padding=PADDING),
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),
            nn.Conv2d(128, 128, kernel_size=KERNEL, padding=PADDING),
            nn.BatchNorm2d(128),
            ...
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True),
            nn.Conv2d(32, in_channels, kernel_size=KERNEL, padding=PADDING),
            nn.Sigmoid(),
        )
...
class VAEDecode_down2(nn.Module):
    def __init__(self, KERNEL=3, PADDING=1, in_channels=1, num_classes = 4):
        ...
        self.decoderB=nn.Sequential(
            nn.Conv2d(128 + num_classes, 128, kernel_size=KERNEL, padding=PADDING),
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),
            nn.Conv2d(128, 128, kernel_size=KERNEL, padding=PADDING),
            nn.BatchNorm2d(128),
...
class VAEDecode_down4(nn.Module):
    def __init__(self, KERNEL=3, PADDING=1, in_channels=1,num_classes = 4):
        ...
        self.decoderB=nn.Sequential(
            nn.Conv2d(256 + num_classes, 128, kernel_size=KERNEL, padding=PADDING),
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),
            nn.Conv2d(128, 128, kernel_size=KERNEL, padding=PADDING),
            nn.BatchNorm2d(128),
